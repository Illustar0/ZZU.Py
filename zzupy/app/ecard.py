...
from zzupy.exception import LoginError, NetworkError, OperationError, ParsingError, NotLoggedInError
...
class ECardClient:
    ...
    def __init__(self, cas_client: ICASClient) -> None:
        ...
        :param cas_client: 已登录的 CASClient 实例
        """
        if cas_client.user_token is None:
            raise LoginError("CASClient 必须已经登录")
    ...
    def _get_tid(self) -> None:
        ...
        :raises ParsingError: 如果响应解析失败
        :raises NetworkError: 如果网络请求失败
        """
        params = {
            "host": "11",
            "org": "2",
        ...
        try:
            ...
            raise NetworkError(f"服务器返回错误状态 {exc.response.status_code}") from exc
    ...
    def get_default_room(self) -> str:
        ...
        if not self._access_token:
            logger.error("尝试获取默认房间但未登录")
            raise NotLoggedInError("必须先登录")
    ...
    def recharge_energy(
        ...
        :param amt: 充值金额
        :param room: 房间 ID 。格式应为 'areaid-buildingid--unitid-roomid'，可通过 get_room_dict() 获取
        :raises OperationError: 如果充值失败
        :raises ParsingError: 如果响应解析失败
        :raises NetworkError: 如果网络请求失败
        """
        if not self._access_token:
            logger.error("尝试充值但未登录")
            raise LoginError("必须先登录")
    ...
    def get_room_dict(self, room_id: str) -> dict:
        ...
        :param room_id: 已知房间 ID 。例如: '', '99', '99-12', '99-12--33'
        :return: 对应的字典
        :raises ValueError: 如果参数格式不正确
        :raises ParsingError: 如果响应解析失败
        :raises NetworkError: 如果网络请求失败
        """
        if not self._access_token:
            logger.error("尝试获取房间列表但未登录")
            raise NotLoggedInError("必须先登录")
